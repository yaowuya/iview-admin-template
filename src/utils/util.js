...
import { forEach, hasOneOf, objEqual } from './tools.js'
...
export const parseUploadUrl = (url) => {
  if (process.env.NODE_ENV === 'development') {
    return config.devProxyTarget + url
  } else {
    return window.siteUrl + url
  }
}

// 防抖debounce代码：在事件被触发n秒后再执行回调函数，如果在这n秒内又被触发，则重新计时。
...
// 节流throttle代码:规定一个单位时间，在这个单位时间内，只能有一次触发事件的回调函数执行，如果在同一个单位时间内某事件被触发多次，只有一次能生效。
